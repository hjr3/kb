...
class QueryRequest(BaseModel):
    ...
    question: str = Field(
        ...
        json_schema_extra={"example": "What were my thoughts on microservices architecture?"}
    )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not vault_path:
            logger.error("knowledge_base_initialization_failed", error="OBSIDIAN_VAULT_PATH environment variable not set")
            raise ValueError("OBSIDIAN_VAULT_PATH environment variable must be set")
            
        if not anthropic_api_key:
            logger.error("knowledge_base_initialization_failed", error="ANTHROPIC_API_KEY environment variable not set")
            raise ValueError("ANTHROPIC_API_KEY environment variable must be set")
            
        app.state.rag = KnowledgeBase(
